...
const WarningPopup = ({ toolName, daysLeft, onClose }) => {
  // 2. ใช้ createPortal เพื่อส่ง Popup ไปแสดงที่ชั้นนอกสุด (body) เพื่อให้ทับ Navbar ได้ 100%
  return createPortal(
    /* 3. เปลี่ยนจาก absolute เป็น fixed และใช้ z-[9999] ที่สูงกว่า Navbar แน่นอน */
    <div className="fixed inset-0 z-[9999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      ...
      <div className="bg-[#1e1e1e] border border-gray-700 rounded-xl shadow-2xl p-7 w-full max-w-md text-center relative overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <button className="bg-[#ff9900] hover:bg-[#e68a00] text-black font-bold py-3 px-6 rounded-lg w-full transition flex items-center justify-center gap-2 mb-4 shadow-[0_0_15px_rgba(255,153,0,0.3)]">
    ...
    document.body // ส่ง Component นี้ไปที่ body
  );
};

...
// 🟢 สร้างการ์ดพิเศษสำหรับโหมด 2 ช่อง (สรุปภาพรวม SET และ MAI)
const SET_SUMMARY = {
  id: "SET_ALL",
  label: "SET (Overall)",
  market: "SET",
...
const IconX = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#64748b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>;
const IconMaximize = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg>;
const IconMinimize = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3v3a2 2 0 0 1-2 2H3"/><path d="M21 8h-3a2 2 0 0 1-2-2V3"/><path d="M3 16h3a2 2 0 0 1 2 2v3"/><path d="M16 21v-3a2 2 0 0 1 2-2h3"/></svg>;
  ...
  return (
    <div ref={dropdownRef} style={{ position: "relative", zIndex: 150 }}>
      ...
      <div
        ...
        style={{
          display: "flex", alignItems: "center", justifyContent: "space-between",
          background: "#0d1520", border: "1px solid rgba(51,65,85,0.6)",
      ...
      {isOpen && (
        <div style={{
          ...
          <div style={{ padding: "10px 14px", borderBottom: "1px solid rgba(51,65,85,0.4)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
             ...
             <div style={{ transform: "rotate(180deg)", cursor: "pointer" }} onClick={() => setIsOpen(false)}>
               <IconCaret />
             </div>
          </div>
...
function ExpandedChart({ sector, dateVal, tradingDates, onClose, onFirst, onLast }) {
  const canvasRef = useRef(null);
  const [hoverX, setHoverX] = useState(null);

  // สร้างข้อมูลแต่ละ stock (แยกสีต่างกัน)
  const COLORS = ["#22d3ee","#f97316","#a78bfa","#f472b6","#4ade80","#facc15","#38bdf8","#fb7185"];
  const stockLines = useMemo(() => {
    return sector.stocks.map((st, i) => ({
      ...st,
      color: COLORS[i % COLORS.length],
      vals: genSectorData(st.sym, 80),
    }));
  }, [sector]);

  // Time labels (จำลอง intraday 10:00–15:30)
  ...
  useEffect(() => {
    ...
    if (!canvas || stockLines.length === 0) return;
    ...
    const padL = 20, padR = 110, padT = 20, padB = 30;
    const chartW = W - padL - padR;
    const chartH = H - padT - padB;

    // หา min/max รวมทุก stock
    const allVals = stockLines.flatMap(s => s.vals);
    ...
    for (let i = 0; i <= 8; i++) {
      ...
      const val = mx - (range / 8) * i;
      ctx.fillStyle = "#475569"; ctx.font = "10px monospace";
      ctx.textAlign = "right"; ctx.fillText(val.toFixed(2), W - padR + 8 + 30, y + 3);
    }
    for (let i = 0; i <= 6; i++) {
    ...
    timeLabels.forEach((t, i) => {
      if (i % step !== 0) return;
      const x = padL + (i / (timeLabels.length - 1)) * chartW;
      ctx.fillText(t, x, H - padB + 16);
    });
    ...
    stockLines.forEach(stock => {
      ...
      ctx.lineWidth = 1.5;
      ctx.lineJoin = "round";
      stock.vals.forEach((v, i) => {
        const x = padL + (i / (stock.vals.length - 1)) * chartW;
  ...
  return (
    <div style={{
      ...
      <div style={{
        display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "10px 20px", borderBottom: "1px solid rgba(51,65,85,0.5)",
        flexShrink: 0,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button style={{
            ...
            borderRadius: 6, padding: 6, color: "#64748b", cursor: "pointer",
          }}>
            <IconRefresh />
          </button>
          <span style={{ fontSize: 20, fontWeight: 700, color: "#e2e8f0", letterSpacing: ".05em" }}>
            {sector.label}
          </span>
          {sector.stocks.some(s => s.hasFlow) && (
            <span style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={onFirst} style={{
              background: "transparent", border: "1px solid #0f766e",
              borderRadius: 4, padding: "0 14px", height: 32, fontSize: 11,
              fontWeight: 600, cursor: "pointer", color: "#2dd4bf",
            }}>FIRST DATA</button>
            <button onClick={onLast} style={{
              background: "transparent", border: "1px solid #0f766e",
              borderRadius: 4, padding: "0 14px", height: 32, fontSize: 11,
              fontWeight: 600, cursor: "pointer", color: "#2dd4bf",
            }}>LAST DATA</button>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 11, color: "#64748b" }}>Start Graph</div>
            <div style={{ fontSize: 11, color: "#94a3b8", fontFamily: "monospace" }}>10:00–15:30</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <div style={{ flex: 1, position: "relative" }}
          onMouseMove={e => {
            ...
            const padL = 20, padR = 110;
            const chartW = rect.width - padL - padR;
            const x = (e.clientX - rect.left - padL) / chartW;
        ...
        <div style={{
          ...
          {stockLines.map(st => {
            const lastVal = st.vals[st.vals.length - 1];
            return (
              <div key={st.sym} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
                <span style={{
                  background: st.color, color: "#000", fontSize: 11, fontWeight: 700,
                  padding: "2px 7px", borderRadius: 4, minWidth: 48, textAlign: "center",
                }}>{st.sym}</span>
                <span style={{ fontSize: 13, fontFamily: "monospace", color: "#e2e8f0", fontWeight: 500 }}>
                  {((lastVal / 10) + 80).toFixed(2)}
                </span>
              </div>
            );
          })}
...
// ─── MINI CHART CANVAS ────────────────────────────────────────────────────────
function SectorMiniChart({ sectorId, isUp, isEmpty, isExpanded, dateVal }) {
  const canvasRef = useRef(null);
  ...
  // คำนวณวันที่จำลอง (Simulated Date) ตามตำแหน่งสัดส่วนของข้อมูล
  ...
  // 1. สร้างรอบข้อมูลแบบคงที่ เพื่อให้เวลาเปลี่ยนวันที่ หน้าตากราฟจะไม่ Random เด้งไปมา
  useEffect(() => {
    let newVals = genSectorData(sectorId, 160);
    setVals(newVals);
  }, [sectorId]);

  // 2. เมื่อวันที่เปลี่ยน กระตุ้นให้กราฟเลื่อนตัว (Scroll) ไปหาตำแหน่งของวันนั้น (ซ้ายสุด=Aug 2021, ขวาสุด=Today)
  useEffect(() => {
    if (!wrapperRef.current) return;
    const wrap = wrapperRef.current;
    
    setTimeout(() => {
        // เทียบอัตราส่วนวันที่เพื่อวัดว่าต้อง Scroll ไปกี่เปอร์เซ็นต์
        const start = new Date("2019-01-02").getTime();
        const end = new Date().getTime();
  ...
  const handleMouseMove = (e) => {
    ...
    // คำนวณ Padding เดียวกับ Canvas
    const padL = isExpanded ? 20 : 0;
    const padR = isExpanded ? 20 : 0;
    const chartW = W - padL - padR;
    
    // หาตำแหน่ง Index ของข้อมูลที่ใกล้เมาส์ที่สุด
    ...
    // แปลง Index ออกมาเป็นพิกัดแกน X, Y
  ...
  return (
    <div ref={wrapperRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", overflowX: "auto", overflowY: "hidden", scrollbarWidth: "none", cursor: "crosshair" }}
      onMouseDown={e => {
        ...
        const onMouseMoveDrag = (moveE) => {
          ...
          setHoverInfo(null); // ซ่อนลอจิก Tooltip ระหว่างลาก
        };
    ...
    >
      ...
      >
        ...
        {/* กล่อง Tooltip ที่เด้งขึ้นมาเวลาเอาเมาส์ชี้ */}
        {hoverInfo && (
          <>
             {/* เส้นประ Crosshair ลากลงมาจากจุดยอด */}
             ...
             {/* จุดกลม (Dot Indicator) วางทับบนเส้นกราฟ */}
...
// ─── SECTOR CARD ──────────────────────────────────────────────────────────────
function SectorCard({ sector, isExpanded, onExpand, onCollapse, dateVal }) {
  ...
  return (
    <div style={{
      background: "#0d1520", border: isExpanded ? "none" : "1px solid rgba(51,65,85,0.6)", borderRadius: 8,
    ...
    onMouseEnter={e => { if(!isExpanded) e.currentTarget.style.borderColor = "rgba(100,116,139,0.8)" }}
    onMouseLeave={e => { if(!isExpanded) e.currentTarget.style.borderColor = "rgba(51,65,85,0.6)" }}
    >
      {/* Top Header Row */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isExpanded ? "0 20px" : "0 16px", marginBottom: isExpanded ? 20 : 12, zIndex: 10 }}>
         ...
         <div style={{ display: "flex", gap: 8 }}>
            ...
            <button style={{ background: "transparent", border: "none", color: "#64748b", cursor: "pointer", padding: 2 }}><IconRefresh /></button>
         </div>
      </div>
      ...
      {isEmpty ? (
        ...
        <div style={{ display: "flex", flex: 1 }}>
           {/* Chart */}
           <div style={{ flex: 1, position: "relative", minWidth: 0, borderRight: "1px solid rgba(51,65,85,0.4)" }}>
              <SectorMiniChart sectorId={sector.id} isUp={isUp} isEmpty={isEmpty} isExpanded={isExpanded} dateVal={dateVal} />
           </div>

           {/* Stock List (Right Edge) */}
...
export default function SectorRotation() {
  ...
  // 🟢 โหมดให้ Card ขยายเต็มหน้าจอ (Expanded)
  const [expandedSectorId, setExpandedSectorId] = useState(null);
  
  // 🟢 ควบคุมโหมดตลาดหลัก (SET / MAI / SET&MAI)
  ...
  // 🟢 ฟังก์ชันอัปเดตเมื่อกดปุ่ม (29, 8, 2) หรือกด Dropdown เพื่อให้ทำงานสัมพันธ์กัน
  ...
  // ดึงรายชื่อ Sector ทั้งหมดมาเป็นตัวเลือกใน Dropdown (อิงตาม Market ที่เลือก)
  ...
  // 🟢 กรองข้อมูลที่จะแสดงผลใน Grid (กรณี 29, 8, 2 จะออกผลลัพธ์ต่างกันชัดเจน)
  const visibleData = useMemo(() => {
    let data = [];
    if (marketFilter === "SET") data = SET_SECTORS;
    else if (marketFilter === "MAI") data = MAI_SECTORS;
    else if (marketFilter === "SET&MAI") data = [SET_SUMMARY, MAI_SUMMARY]; // โชว์แค่ 2 การ์ด
  ...
  // ใช้ CSS Grid auto-fit เพื่อจัดเรียงกล่องให้ขยายเต็มขนาดหน้าจอแบบจำกัดคอลัมน์สูงสุดแบบ Responsive
  const gridTemplate = useMemo(() => {
    // max(30%, 300px) คุมให้กว้างสุด 3 กล่อง/แถว (เพราะแต่ละอันกินเนื้อที่อย่างน้อย 30%) จะใหญ่ขึ้นมาพองดงาม
  ...
  // Styles
  const inputStyle = { background: "transparent", border: "none", outline: "none", color: "#e2e8f0", fontSize: 13, width: "100%", cursor: "pointer" };
  const boxStyle = { display: "flex", alignItems: "center", gap: 8, background: "#0d1520", border: "1px solid rgba(51,65,85,0.6)", borderRadius: 6, padding: "0 12px", height: 36, position: "relative", cursor: "pointer" };
  const btnStyle = (active) => ({
    background: active ? "rgba(51,65,85,0.8)" : "transparent", border: "1px solid rgba(51,65,85,0.6)", borderRadius: 6,
  ...
  const dataBtn = (label, onClick) => (
    <button key={label} onClick={onClick} style={{ background: "transparent", border: "1px solid #0f766e", borderRadius: 4, padding: "0 14px", height: 34, fontSize: 11, fontWeight: 600, cursor: "pointer", color: "#2dd4bf", letterSpacing: ".04em", transition: "all .15s" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(45,212,191,0.1)"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>{label}</button>
  );

  return (
    <div style={{ background: "#0a0f16", color: "#e2e8f0", fontFamily: "ui-sans-serif,system-ui,sans-serif", minHeight: "100vh", padding: "20px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <div ref={marketDropdownRef} style={{ ...boxStyle, minWidth: 220 }} onClick={() => setMarketDropdownOpen(!marketDropdownOpen)}>
              ...
              <div style={{ display: "flex", alignItems: "center", cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); setMarketFilter("SET"); }}>
                <IconX />
              </div>

              {marketDropdownOpen && (
                <div style={{ position: "absolute", top: "100%", left: 0, marginTop: 4, background: "#0d1520", border: "1px solid rgba(51,65,85,0.6)", borderRadius: 8, padding: "8px 0", width: "100%", zIndex: 200, boxShadow: "0 10px 25px -5px rgba(0, 0, 0, 0.8)" }}>
      ...
      <div style={{
        ...
        {visibleData.map(sec => (
          <SectorCard key={sec.id} sector={sec} dateVal={dateVal} onExpand={() => setExpandedSectorId(sec.id)} />
        ))}
      ...
      {/* ── Expanded Modal Area ── */}
      {expandedSectorId && (() => {
            const sec = visibleData.find(s => s.id === expandedSectorId);
            if (!sec) return null;
            return (
                <ExpandedChart
                sector={sec}

          ...
          {type === "Shareholder" && (() => {
            ...
            return (
              <g transform={`translate(6, ${normalizeY(lastVal)})`}>
                ...
                <text x="21" y="0" fill="#ffffff" fontSize="11" textAnchor="middle" dominantBaseline="central" fontWeight="bold">{lastVal.toFixed(2)}</text>
              </g>
            );
          })()}
      ...
      <div className="relative w-full min-h-screen text-white overflow-hidden animate-fade-in pb-20">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-8 flex flex-col items-center">
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-4">
            <button onClick={() => window.open("/login", "_blank")} className="w-full md:w-auto px-8 py-3 rounded-full bg-slate-800 text-white font-semibold border border-slate-600 hover:bg-slate-700 hover:border-slate-500 transition-all duration-300">Sign In</button>
            <button onClick={() => window.open("/member-register", "_blank")} className="w-full md:w-auto px-8 py-3 rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-bold hover:brightness-110 shadow-lg hover:shadow-cyan-500/25 transition-all duration-300">Join Membership</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="w-full min-h-screen bg-[#0B1221] text-white px-6 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <div className="relative w-80">
            ...
            />
            {symbol && (
              <button onClick={() => { setSymbol(""); setSelectedSymbol(""); setShowDropdown(false); }} className="absolute right-3 inset-y-0 flex items-center text-slate-400 hover:text-red-400 transition">
              ...
              <div className="absolute mt-2 w-full bg-[#0f172a] border border-slate-700 rounded-xl shadow-2xl max-h-72 overflow-y-auto z-50">
                {filteredSymbols.length > 0
                  ? filteredSymbols.map((item, index) => (
                      <div key={index} onMouseDown={() => { setSymbol(item); setSelectedSymbol(item); setShowDropdown(false); }}
